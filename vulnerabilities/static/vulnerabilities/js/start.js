...
function appendLog(text, type = "default") {
    ...
    let lineType = type;
    if (text.startsWith("FALSE")) lineType = "error";
    else if (text.startsWith("OK")) lineType = "ok";
    const colors = { ok: "#00ff00", error: "#ff0000", default: "rgba(255,255,255,0.7)" };
    const line = document.createElement("div");
    line.style.color = colors[lineType];
...
function startStatusPolling() {
    ...
    statusInterval = setInterval(async () => {
        try {
            const resp = await callApi("status");
            if (resp.response && resp.response.trim() !== "") {
                appendLog(resp.response, "status");
            }
        } catch(e) {}
    }, 500);
}
...
window.addEventListener("beforeunload", () => {
    if (isRunning) {
        navigator.sendBeacon("/run", JSON.stringify({ action: "stop" }));
    }
});

actionBtn.addEventListener("click", async () => {
    if (isRunning) {
        appendLog("STOPPING | Attack will be stopped...", "default");
        await stopAttack();
        appendLog("STOPPED | Terminal cleared", "ok");
        return;
    }
    ...
    log.innerHTML = "";
    if (!raw) {
        appendLog("ERROR | Enter a target URL", "error");
        return;
    }
    const url = normalizeUrl(raw);
    appendLog("CHECKING | " + url);
    actionBtn.disabled = true;

    try {
        const checkResp = await callApi("check", { target: url });
        if (checkResp.response.startsWith("ERROR")) {
            appendLog(checkResp.response, "error");
            actionBtn.disabled = false;
            return;
        }
        appendLog("OK | Site is reachable", "ok");
        appendLog("CONNECTING | Reaching backend...");

        const startResp = await callApi("start", { target: url, workers: 500 });
        if (startResp.response.startsWith("OK")) {
            appendLog(startResp.response, "ok");
            startStatusPolling();
            actionBtn.textContent = "STOP";
            isRunning = true;
        } else {
            appendLog(startResp.response, "error");
        }
        actionBtn.disabled = false;
    } catch (e) {
        appendLog("ERROR | Backend not reachable: " + e.message, "error");
        actionBtn.disabled = false;
    }
});